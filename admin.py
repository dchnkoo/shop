...
class Admin(Card):
    ...
    def random_art(self) -> int:
        return int(''.join(str(randint(0, 9)) for _ in range(6)))
             
    
    def insert_product_info(self, **kwargs):

        article = self.random_art()
    ...
    def auth(self):
        return self.__db.execute("SELECT * FROM login WHERE for=?", 'admin')[0]